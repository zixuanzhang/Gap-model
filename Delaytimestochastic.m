...
%         tn/tnmax
    ...
    edge = resultu(600,:);
    for cell = 1:n
        if edge(cell) > 0.2 & edge(cell+1)<0.2
        proptime = cell;
        break
        end
    end
    resulttrans = [resulttrans proptime];
end
% [m,n] = size(resultu);
% edge = resultu(600,:);
% for cell = 1:n
...
%         for ti = 1:round(tn/100)
%             if resultu(5,ti) > 0.2 & resultu(60,ti+1) < 0.2
%                 countpro = countpro + 1;
%             end
% 
...
% nch60_100= [count10,count11,count21,count22,count32,count33]
% freq_nch60_100 = nch60_100/itr
